...
/* Set seed for random number generator equal to task ID */
   ...
   */
   ...
   MPI_Comm_size(row_comm,&numtasks_col);
   MPI_Comm_rank(row_comm,&taskid_col);
 
  
  //Define column groups

   MPI_Comm_split(MPI_COMM_WORLD,coor_ij[1],coor_ij[0],&col_comm);
   MPI_Comm_size(col_comm,&numtasks_row);
   MPI_Comm_rank(col_comm,&taskid_row); //col_comm goes for all the rows
   ...
   for(k=0;k<numbelem;k++){
      ...
      if(k_owner==coor_ij[1]){
         ...
         if(taskid_col>0)
            MPI_Isend(col_deposit, numelem_p, MPI_INT, taskid_col-1, 0, row_comm, &request_row);
         if(taskid_col<(numtasks_col-1))
            MPI_Isend(col_deposit, numelem_p, MPI_INT, taskid_col+1, 0, row_comm, &request_row);
        
      }else{
         if(k_owner>taskid_col){
            MPI_Recv(col_deposit,numelem_p, MPI_INT, taskid_col+1, 0, row_comm, MPI_STATUS_IGNORE);
      ...
      if(k_owner==coor_ij[0]){
         ...
         if(taskid_row>0)
            MPI_Isend(row_deposit, numelem_p, MPI_INT, taskid_row-1, 0, col_comm, &request_col);
         if(taskid_row<(numtasks_row-1))
            MPI_Isend(row_deposit, numelem_p, MPI_INT, taskid_row+1, 0, col_comm, &request_col);
         
      }else{
         if(k_owner>taskid_row){
            MPI_Recv(row_deposit,numelem_p, MPI_INT, taskid_row+1, 0, col_comm, MPI_STATUS_IGNORE);
            if(taskid_row>0)
               MPI_Isend(row_deposit, numelem_p, MPI_INT, taskid_row-1, 0, col_comm, &request_col);
         }else{
            MPI_Recv(row_deposit,numelem_p, MPI_INT, taskid_row-1, 0, col_comm, MPI_STATUS_IGNORE);
